...
> 当前代码版本：commit `a2475e7`（真实 LLM 修复批次，2026-07-15）
...
### 10.4 第三轮验证（D1 Router fallback + D3 cost 浮点改造后）

> 计划但未执行。本轮应重点验证 D1 后 multi-agent 场景3 是否从 `failed`（max_steps / 403 死循环）变为 `completed`，并检查 D3 后单次 dialogue 的 `/api/costs` 是否返回非 0 的 `total_cost_usd`（预期约 $0.001~$0.01 级，3 位小数显示）。
>
> 已提交代码（commit `a2475e7`）：
> - **D1**：main.go:241-258 注册 DefaultProfiles 克隆时把非 cfg.LLMModel 的 tier profile FallbackModel 重定向到 `cfg.LLMModel`，multi-agent intent 选 pro 后 fallback 到 token 可访问的本地模型。
> - **D3**：`CalculateCost` 改 `float64` USD 存储不截断，`CostUSD` 为主字段，`CostCents=int64(round(CostUSD*100))` 为 derived 兼容字段；`/api/costs.total_cost_usd` 直接输出 `float64`。
>
> 执行命令：
> ```bash
