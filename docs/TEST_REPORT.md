...
- **状态**：🔄 修复中（task #3）。价格已查到权威值（DeepSeek 官方），DefaultProfiles 修正 + 前端价格编辑 UI
...
| `reasoning` 字段处理 | `openai_provider.go` / `client.go` | mock 不含 reasoning |
...
- cost model 名对齐 `deepseek-v4-flash-local`（R4-OnLLMUsage 修复），usd 仍 0 待 task #3 修价格
