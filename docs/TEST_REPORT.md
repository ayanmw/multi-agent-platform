...
4. **[低危] `handleAPIKeyByID` 错误的 `errors.Is` 判断**（`auth_http.go:236`）：`errors.Is(err, errors.New("api key not found"))` 永远 false，靠 `strings.Contains` 兜底才工作。代码异味（code smell）。
...
| 场景 | 终态 | 关键证据 |
|------|------|---------|
| 1 write_file | completed | 3s，tool_calls=1（write_file），hello.txt 落盘，tokens=2305 |
| 2 run_shell | completed | 2s，tool_calls=1（run_shell），无 POLICY BLOCK |
| 3 multi-agent(2) | **completed** | 87s， Router fallback chain 正确：deepseek-v4-pro 403 → deepseek-v4-flash-local；无 web_search 依赖错误、无 malformed JSON 400 |
| 4 dialogue | completed | 7s，tokens=1376，`total_cost_usd=0.0002667`（D3 浮点生效） |
| 5 run-case | completed | 22s，dialogue case 正常 |
...
D3 验证结论：
- ✅ `/api/costs.total_cost_usd` 返回非 0 浮点值（0.0002~0.00085 USD 级），小对话不再截断为 0
- ✅ cost 记录链路完整

本轮修复的真实问题：

#### ✅ R7【严重】multi-agent researcher 系统提示硬依赖未注册的 `web_search` 工具 — 已修复
- **位置**：`internal/orchestrator/orchestrator.go` TaskDecomposer 的 researcher prompt
- **修复**：prompt 改为使用已注册工具（`read_file` / `write_file` / `run_shell`），不再要求 `web_search`
- **验证**：场景 3 multi-agent `status=completed`，researcher 不再因 `tool not found: web_search` 失败

#### ✅ R8【中】malformed tool_call 被原样回传到 conversation history 导致 API 400 — 已修复
- **位置**：`internal/runtime/engine.go` 错误分支与成功分支将 `tc` 原样 append 到 `e.messages`
- **修复**：新增 `sanitizeToolCallArguments()` / `isValidToolArgumentsJSON()`；每次把 tool_call 写回 conversation history 前校验 Arguments 是否为合法 JSON，非法时替换为 `{"_error":"invalid arguments JSON"}` 占位对象
- **验证**：服务日志 `Repaired malformed arguments JSON for write_file` 后未再触发 400 Unterminated string，场景 3 完成
