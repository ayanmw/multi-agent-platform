...
// Version 返回该 proxy tool 的版本标识符。MCP tool 默认无版本。
func (t *ProxyTool) Version() string { return "" }

// Source 返回该 proxy tool 的来源。MCP proxy tool 始终返回 "mcp"。
func (t *ProxyTool) Source() string { return "mcp" }

// CanonicalName 返回 Registry 使用的唯一键。无版本时等于 FullName()。
func (t *ProxyTool) CanonicalName() string {
	if v := t.Version(); v != "" {
