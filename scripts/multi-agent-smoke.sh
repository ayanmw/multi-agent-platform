...
echo "  5. [已修复] 子任务 status/steps 回填: handleGetTask 已返回 child_tasks.steps，"
echo "     前端 loadTask 已把 worker steps 回填到对应 lane。"
...
echo "  7. [已修复] OutputTo 通用路由已启用: AgentSpec.OutputTo 声明 researcher -> writer，"
echo "     即使 parallel 策略下，agent 完成后也会把结果发给 OutputTo 中的目标 agents。"
echo "     AgentBus  queue 会自动处理目标尚未注册 handler 的情况。"
echo "  8. [已修复] root task 终态由 RunBlocking 显式 UpdateTask: multi-agent 并行状态"
echo "     不再依赖轮询，orchestrator 汇总后直接更新 root task。"
echo "  9. [已修复] 编排层可观测: RunBlocking 现在发送 decompose_done / agent_dispatched /"
echo "     agent_completed 事件(agent_id='orchestrator')，前端有独立 orchestrator lane。"
