...
echo "  6. [仍存] AgentBus 未实际使用: engine.sendAgentMessage 方法定义但无调用方，多 agent"
echo "     间无消息传递。researcher 成果不会传给 writer，writer 独立执行。"
echo "  7. [仍存] strategy 字段无效: TaskDecomposer 返回 Strategy=\"pipeline\"/\"parallel\"，"
echo "     但 RunBlocking 无视 Strategy，总是并行启动所有 agent。"
