#!/usr/bin/env bash
# =============================================================================
# Multi-Agent Platform — 多 Agent 编排专项评测脚本 (维度 C) [Auth-on]
# =============================================================================
# 评测 POST /api/multi-agent 的 orchestrator 拆分、子任务派发、child_tasks
# 持久化、多 agent 事件区分等能力。使用 mock LLM 确定性运行。
# 本脚本为 multi-agent-smoke.sh 的 auth-on 版本： REQUIRE_AUTH=true，
# POST 请求均携带从启动日志提取的默认 admin Bearer token。
#
# 测试项：
...
DB_PATH="/tmp/ma-smoke-auth-$$.db"
SERVER_BIN="/tmp/ma-smoke-auth-server-$$.exe"
SERVER_LOG="/tmp/ma-smoke-auth-server-$$.log"
SERVER_PID=""
...
cleanup() {
  ...
  rm -f /tmp/ma-smoke-auth-*.json 2>/dev/null
}
trap cleanup EXIT
...
ADMIN_KEY=$(grep "DEFAULT ADMIN API KEY" "${SERVER_LOG}" | head -1 | sed -E 's/.*DEFAULT ADMIN API KEY:[[:space:]]*(sk_[A-Za-z0-9_=-]+).*/\1/')
if [[ -z "${ADMIN_KEY}" ]]; then
...
echo ""
if [[ $FAIL -gt 0 ]]; then
  echo "[multi-agent-auth] 存在 FAIL 项，详见上方结果。服务日志：${SERVER_LOG}"
  exit 1
fi
echo "[multi-agent-auth] 评测完成 (PASS=${PASS}, SKIP=${SKIP}, FAIL=${FAIL}) [Auth-on]"
exit 0
