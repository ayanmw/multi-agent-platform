...
1. **Phase 6-C** (本次): 修复 P1 技术债务 + 设计文档
2. **Phase 6-D** (下次): 修复 P2 技术债务 + RAG/Auth/可观测性骨架
3. **Phase 7**: 完整实现 RAG + Auth + 可观测性

**Rollback**: 所有变更为向后兼容，可通过 Git revert 回退。

## Open Questions

- [ ] RAG 检索策略：向量相似度阈值？混合检索（关键词 + 向量）？
- [ ] Auth 多用户支持需求优先级？
- [ ] 可观测性指标格式：Prometheus vs OpenTelemetry？
