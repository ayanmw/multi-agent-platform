...
本 capability 定义多 Agent 编排能力的 case 覆盖契约：编排策略（parallel / sequential / pipeline / DAG / leader-driven dispatch）的可观测事件要求、agent 间消息传递、以及动态派发行为。该契约既是 case 的验收依据，也是 orchestrator 底层变更的回归防线（对应 memory `multi-agent-dual-entry-placeholder-bug` 记录的"root task 空壳 / 子 agent 挂假 taskID"类回归）。
